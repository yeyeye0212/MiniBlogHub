...
def post_detail(post_id):
    post = Post.query.filter_by(post_id=post_id).order_by(Post.created_at.desc()).all()
    comments = Comment.query.filter_by(post_id=post_id).order_by(Comment.created_at.desc()).all()
    return render_template('show.html', post=post,comments=comments,username=session.get('username'))
